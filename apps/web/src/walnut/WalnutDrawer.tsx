...
export function WalnutDrawer({
  ...
  runId: string;
  /** Optional: drive the active tab from outside (the guided tour does this). Uncontrolled when omitted. */
  tab?: WalnutTab;
  onTabChange?: (tab: WalnutTab) => void;
}) {
  const [internalTab, setInternalTab] = useState<WalnutTab>("overview");
  const tab = controlledTab ?? internalTab;

  // Kept in a ref so the run-change reset below stays on [runId] alone.
  const onTabChangeRef = useRef(onTabChange);
  onTabChangeRef.current = onTabChange;

  const setTab = useCallback((next: WalnutTab) => {
    setInternalTab(next);
  ...
  useEffect(() => {
    ...
    onTabChangeRef.current?.("overview");
  ...
  return (
    <div className="walnut-drawer">
      ...
      <div className="walnut-tabs" role="tablist" data-tour="walnut-tabs">
        {tabs.map((item) => (
          <button
            key={item.id}
            type="button"
        ...
        {tab === "overview" &&
          (overview.loading ? (
            ...
            <OverviewPanel
              overview={overview.data}
              runId={runId}
              onReconciled={refreshIncidentState}
            />
          ) : null)}
