...
// -- §A5. AgentVersion resolver (implemented by Claude in context/agent-version-store.ts) --
...
// -- §B3. CitationVerifier hook (implemented by Claude in context/citation-verifier.ts, called
...
export interface CitationVerifier {
  ...
  // only quotePreview + quoteHash (amended 2026-08-27 per B-002).
  verify(input: {
    quote: string;
    charStart: number;
    charEnd: number;
