...
describe("Run overview, evidence timeline, and verify", () => {
  it("returns live Phase-3 summaries, applies knownAt, and verifies both chains", async () => {
    const { app, service } = await makeHarness();
    ...
    expect(overview.evidenceSummary).toEqual({ consumed: 0, denied: 0, produced: 0 });
