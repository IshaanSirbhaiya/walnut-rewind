...
Runs the frozen scenario (`demo/SCENARIO.md`) end-to-end and shows **all 23 capabilities**
(README §18 map) in story order. Every expected output below was captured from a real
...
```bash
# 1. Clean slate (walnut state is disposable demo state; agents/runs history may remain)
rm -rf "$HOME/.volc-agent-launchpad/data/walnut" "$HOME/.volc-agent-launchpad/workspaces"/*/.walnut
...
```bash
curl -sS -X POST $BASE/api/agents/$RESEARCH/messages -H 'Content-Type: application/json' \
...
# capture run id → R1; poll GET /api/runs/$R1 until "completed" (rehearsed: 13 s)
...
curl -sS -X POST $BASE/api/agents/$STRATEGY/messages -H 'Content-Type: application/json' \
  -d '{"content":"Summarize the approved Aurora launch plan."}'          # → run R2
```
...
```bash
curl -sS -X POST $BASE/api/agents/$STRATEGY/messages -H 'Content-Type: application/json' \
  -d '{"content":"Summarize the approved Aurora launch plan, including the launch date and price."}'   # → R3
...
curl -sS -X POST $BASE/api/agents/$COMMS/messages -H 'Content-Type: application/json' \
  -d '{"content":"Read COMMS_TASK.md in your workspace and complete it exactly. Use only the launch evidence in your Walnut context capsule."}'   # → R4
