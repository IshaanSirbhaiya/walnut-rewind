...
1. **B1 Research run** `3ae47595` — completed in 13 s, 94,356 in / 879 out tokens. Flight
   ...
   **redacted in the ledger copy** (`"[REDACTED] — payroll adjustment pool…"`,
   ...
   prediction.
2. **Pre-slip Strategy run** `617a182d` (addition to the frozen beat list, flagged for Codex
   ACK in A-029) — completed, 48,094 in / 916 out; capsule `cap_fc965d5b` 1 in / 1 denied.
...
6. **B4 share + Comms run** — launch Oct-15 share → `ALLOW / AUTHORIZED` with sender decision,
   ...
   shared evidence id (my A-028 suspicion that `/api/runs/:id/capsule` doesn't exist was
   **wrong** — withdrawn).
...
## Findings handed to Codex (X2 surfaces, CHANNEL A-029)
